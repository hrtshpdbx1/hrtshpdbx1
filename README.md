# Hi there 👋 I'm Louise

**Fullstack JavaScript Developer | Accessibility (a11y) & Inclusive UX Advocate**

I build robust, meaningful, and accessible web experiences for everyone. Currently refining my craft at **Interface3** (Brussels), I bridge the gap between high-end visual design and inclusive technical architecture.
🚀 **I am currently looking for an 8 to 12-week internship starting June 2026.** I’m eager to bring my technical skills and my "care" mindset to a team building meaningful and inclusive digital products.
---

### 🔭 I’m currently working on
- **Social Script**: An interaction simulator built with a focus on native semantic navigation and keyboard accessibility (**WCAG AAA**).
- **ReplyToo**: A dynamic platform raising awareness about sexism in the workplace.

### 🌱 I’m currently learning
- Advanced **Web Accessibility** patterns and ARIA best practices.
- Deepening my knowledge of **Node.js** microservices and scalable backend architectures.

### 👯 I’m looking to collaborate on
- **Open Source projects** focused on social impact, digital ethics, or assistive technologies.
- Anything related to **inclusive design** and making the web a safer place for vulnerable communities.

### 💬 Ask me about
- How my background in **social project management** (harm reduction) influences my code.
- Why **Semantic HTML** is the most powerful tool in a developer's kit.
- The intersection of **Digital Arts** (from my Erg/Beaux-Arts days) and clean code.

### 📫 How to reach me
- **LinkedIn**: [https://www.linkedin.com/in/louise-moraldy/]
- **Email**: [lmoraldy.dev@gmail.com]
- **Portfolio**: [https://louisemoraldy.vercel.app/]

### ⚡ Fun fact
When I'm not optimizing keyboard navigation, you'll probably find me wandering the streets of **Brussels**, hunting for the cutest street pups for my project: [📸 **@bxl_petitchien**](https://www.instagram.com/bxl_petitchien/). 

Aside from dog stalking, I’m deeply into **scuba diving**, **Queer Studies**, and **hacker ethics**.
---

### 🛠 Tech Stack & Tools

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![Sass](https://img.shields.io/badge/Sass-CC6699?style=flat-square&logo=sass&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white)
![A11y](https://img.shields.io/badge/Accessibility-WCAG%20AAA-blue?style=flat-square)

---

*“Accessibility is not a feature, it’s the starting point.”*
